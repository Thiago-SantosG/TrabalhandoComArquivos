...
partial class Program
{
    static void LidandoComFileStreamDiretamente()
    {
        ...
        {
            var numeroDeByterLidos = -1;

            var buffer = new byte[1024]; //1KB

            numeroDeByterLidos = fluxoDoArquivo.Read(buffer, 0, 1024);

            while (numeroDeByterLidos != 0)
            {
                numeroDeByterLidos = fluxoDoArquivo.Read(buffer, 0, 1024);
                Console.WriteLine($"Bytes Lidos: {numeroDeByterLidos}");
                EscreverBuffer(buffer, numeroDeByterLidos);
            }
